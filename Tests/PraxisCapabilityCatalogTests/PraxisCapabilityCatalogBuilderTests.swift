...
struct PraxisCapabilityCatalogBuilderTests {
  ...
  func buildThinCapabilityBaselinePublishesPhaseThreeManifestSet() {
    ...
    #expect(baseline.manifests.count == 17)
    #expect(snapshot.families.map(\.name) == ["batch", "code", "embed", "file", "generate", "search", "session", "shell", "skill", "tool"])
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("code.patch"))
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("code.run"))
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("code.sandbox"))
    ...
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("shell.approve"))
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("shell.run"))
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("skill.list"))
    #expect(snapshot.entries.map(\.manifest.id.rawValue).contains("skill.activate"))
