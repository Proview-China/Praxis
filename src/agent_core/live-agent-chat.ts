...
function buildCoreUserInput(input: {
  ...
  return [
    ...
    input.forceFinalAnswer
      ? ""
      : "If shell.restricted, shell.session, test.run, repo.write, code.edit, code.patch, code.diff, git.status, git.diff, write_todos, code.symbol_search, code.lsp, search.web, search.fetch, or search.ground is already registered, treat it as ready-to-use TAP inventory rather than something that still needs user approval.",
