...
- `live-agent-chat` 在第一轮拆分后已经补上 `shared.test.ts`，当前 gate 不再是 baseline only。
- `tap-tooling-adapter` 在第一轮拆分后已经补上 parser / browser / command-runtime / normalizers 四组内部 focused tests，gate 不再只依赖集成级回归。
