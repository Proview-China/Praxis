...
- 自 `2026-04-13` 起，仓库中被跟踪的旧 TypeScript / Node.js 实现已移除。
- 当前仓库唯一活代码主线是 SwiftPM targets 与对应测试。
- `PraxisCLI`、`PraxisAppleUI`、`PraxisRuntimePresentationBridge` 已从主包拓扑中移除，仓库主线继续收成 pure framework + FFI。
- `PraxisRuntimeKit` 已作为高层 Swift API target 落地，当前公开面已收敛为 `runs` / `tap` / `cmp` / `mp` 四个稳定入口；project-scoped API 优先暴露 `overview`、`approvals`、`memory(...)` 这类成组入口，参数面也继续收成 RuntimeKit 自己的 typed input / options model，并将核心标识符进一步收成 RuntimeKit-owned ref types。
- 本文中仍出现的 `src/**`、`npm`、`dist/` 等内容，除非另有说明，都应视为历史谱系引用，而不是当前仓库内仍存在的实现。
...
- 当前仓库内的可验证行为基线已经切到 Swift 主线。
- 旧 TypeScript runtime / facade / integration 代码已从仓库移除，不再作为活实现面存在。
- 若某些 boundary descriptor、计划表或 archive 文档仍提到 `src/agent_core/*`、`src/rax/*`、`src/integrations/*`，它们只表示历史实现来源，不表示这些目录仍然在仓库中维护。
...
- 当前 Swift 代码已经稳定表达：
  - 子域边界
  - Host 分层
  - RuntimeInterface / FFI 入口规则
  - 宿主无关的 typed request / response / summary / facade surface
- Swift 包当前保留的主 runtime surface 为：
...
- 这轮非 UI 收口已经补完 Wave 6 truth / provenance residual、SQLite schema policy、Wave 7 FFI boundary hardening 与旧 TS 跟踪实现剔除
- 但它还不是 TS 运行时的功能等价替代，也还没有正式冻结导出层 ABI 与完整 live host 深度
- 旧 TS 侧现在只保留为历史谱系信息，不再是仓库内可执行参考基线

## 3. 总目标

Swift 重构的目标不是把旧 TypeScript 代码逐文件翻译成 Swift，而是把 framework 主线拆成四层：

1. 可导出的 `PraxisCore`
2. 可替换的 `PraxisHostContracts`
3. 可装配的 `PraxisHostRuntime`
4. 可演进的 framework / export surface：
   - `PraxisRuntimeGateway`
   - `PraxisRuntimeKit`
   - `PraxisFFI`
...
- 仓库主交付收敛成 pure framework，而不是内建 CLI / GUI / TUI 壳
...
2. `PraxisHostContracts`
   - 只放宿主能力协议
3. `PraxisHostRuntime`
   - 只负责装配、use case、facade、runtime interface、gateway
4. `Entry`
   - 只负责 framework public surface 与导出适配
   - framework caller / 跨语言 host 优先走 runtime interface / gateway / FFI
...
### 5.5 Framework / 导出优先 RuntimeInterface 与 FFI

- framework caller 与未来导出 lib / 跨语言 host，不应越过 `PraxisRuntimeInterface` / `PraxisRuntimeGateway` / `PraxisFFI` 直连 Core 或 HostContracts。
- 这条规则不应被理解为“所有未来宿主都必须在这个仓库内自带 UI / CLI”。
- 对未来其它语言实现的 UI / shell / desktop host，优先通过：
  - `PraxisRuntimeInterface`
  - `PraxisRuntimeGateway`
  - `PraxisFFIBridge`
  - 已正式 target 化的 `PraxisFFI`
  进入系统，而不是反向要求主仓库提供 CLI / GUI 壳。
...
| 36 | `PraxisRuntimeGateway` | framework-first bootstrap / runtime access surface / registry factory，负责把 HostRuntime 装成稳定入口 | `src/agent_core/runtime.ts`、`src/rax/runtime.ts` 的中立装配入口 |

### 8.7 Export Surface

| 顺序 | Target | 职责范围 | 说明 |
| --- | --- | --- | --- |
| 37 | `PraxisFFI` | 多语言导出层与宿主接入主路径 | 当前已正式 target 化，并以 `PraxisFFIBridge` 的最小 encoded bridge surface 锁住可用性；ABI / streaming / ownership 细节尚未冻结 |
| 38 | `PraxisRuntimeKit` | 面向 Swift 调用者的高层 framework API，负责默认 runtime 创建，并通过 `runs` / `tap` / `cmp` / `mp` 四个入口承接执行与 inspection；project-scoped surface 以 `overview`、`approvals`、`memory(...)` 为主，参数面优先使用 RuntimeKit 自己的 typed input model 与轻量 ref type | 当前已刻意不暴露 transport envelope、FFI handle、composition/bootstrap 细节、重复顶层 helper、大段位置参数签名或裸字符串标识符 |
...
- `PraxisRuntimeComposition`、`PraxisRuntimeUseCases`、`PraxisRuntimeFacades`、`PraxisRuntimeInterface`、`PraxisRuntimeGateway` 已形成持续被真实测试覆盖的 HostRuntime 闭环。
- 当前已经具备：
...
- 当前阶段应刻意保留的弹性包括：
  ...
  - 最终的句柄释放与缓冲区所有权模型
- 当前本地验证快照见 `2.3`：
  - `swift test` 为 `361` tests / `51` suites 全通过
...
- 正式把系统装起来，并提供 CMP / MP inspection 这类稳定 facade / use case / gateway 表面

完成标准：

- framework caller 与导出层可经由 facade / interface / gateway 调 Core
...
- 当前阶段不再在主仓库内承载 CLI / GUI / TUI 产品实现
- 当前已把现有 `PraxisFFIBridge` 升格成正式 `PraxisFFI` target；进入 Wave 7 时优先继续收紧它的稳定导出边界
- 如果后续 UI 有其它语言代码进入，默认优先建立：
  - `RuntimeInterface` request/response/event contract
  - `RuntimeGateway` bootstrap / registry surface
  - `PraxisFFI` 导出层
  而不是要求主仓库继续自带 Swift CLI 或 SwiftUI 壳
...
- 当前这轮非 UI 收口已完成的关键项包括：
  ...
  - 主仓库已移除旧 TS 跟踪实现，并开始把内建 CLI / GUI / TUI 壳从 package topology 与根计划中剥离出去
...
- 建立可被多宿主复用的 framework / export 边界，而不是先把产品壳做厚

完成标准：

- 主仓库不再绑定 `PraxisCLI` / `PraxisAppleUI` / `PraxisRuntimePresentationBridge`
- 新的 UI / shell 若由其它语言实现，能够优先建立在 `PraxisRuntimeInterface` / `PraxisRuntimeGateway` / `PraxisFFI` 之上
...
- 最容易重新长成新的总装入口，或者把 framework public API 与 transport/export 细节重新混在一起
...
9. PR 9
   ...
   - 状态：已完成（`2026-04-11`）
   - 完成内容：
     - 已形成 facade / runtime interface / gateway / encoded bridge 的最小闭环
...
3. Wave 7 non-UI export/entry hardening
   ...
   - `PraxisRuntimeGateway` 继续承担 framework-first bootstrap，不再把 CLI / GUI 壳绑回主拓扑
4. 仓库主线清理
   - 旧 TS 跟踪实现已移除，CLI / GUI / TUI 壳已进入剥离阶段
...
3. 面向 Swift 调用者的更友好高层 framework API
   - 当前仍主要是 transport-style `RuntimeInterface` 与 export-style `PraxisFFI`
4. 外部宿主体验
   - UI / shell 可在仓库外建立，不属于当前这份 pure framework 主线

最重要的一句话：

- 先证明“Swift 可以承接纯核心”，再证明“Swift 可以装配整个系统”，最后再把它磨成对调用者友好的 framework。
