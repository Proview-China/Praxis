...
Praxis 是一个基于 Swift + SwiftPM 的本地 agent runtime framework，当前主线面向 Swift / Apple host embedding 与可导出的 runtime surfaces。对大多数 Swift 调用方，默认公开入口是 `PraxisRuntimeKit`；当前更适合把它评估为一组正在收束的集成型 runtime products，而不是已经成熟的通用跨平台 agent framework。
...
- 外部评估清单见 [docs/PraxisEvaluationChecklist.md](./docs/PraxisEvaluationChecklist.md)。
- 原生 demo host 说明见 [docs/PraxisDemoHost.md](./docs/PraxisDemoHost.md)。
...
- `PraxisRuntimeKitSmoke --suite code`
  展示 Phase 5 第一条 risky but controlled bounded code 路径；macOS 当前执行 bounded Swift snippet，Linux 诚实返回 placeholder failed-to-launch 语义。
- `PraxisRuntimeKitSmoke --suite code-sandbox`
  展示 Phase 5 的 `code.sandbox` declared contract 读面；macOS 当前返回 caller-visible workspace sandbox contract，非 macOS 或工具链未就绪时返回 placeholder。它描述的是 execution contract，不是 OS-level isolation guarantee。
- `PraxisRuntimeKitSmoke --suite code-patch`
  展示 Phase 5 的 risky but controlled `code.patch` 路径；当前 macOS baseline 通过系统 `patch` 应用单文件 patch。
- `PraxisRuntimeKitSmoke --suite shell`
  展示 Phase 5 第一条 risky but controlled bounded shell 路径；macOS 走真实本地 shell，Linux 诚实返回 placeholder failed-to-launch 语义。
- `PraxisRuntimeKitSmoke --suite shell-approval`
  展示 Phase 5 第二条 bounded shell approval 路径；请求、readback 与 fresh-client recovery 都通过 CMP/TAP durable approval state 完成，因此 approval state 是 durable and recoverable。
- `PraxisRuntimeKitSmoke --suite capabilities`
  验证 provider `skill.activate` / `tool.call`、file、batch 等 capability 基线；其中 provider skill/tool audit truth 依赖 successful outcomes，不会把 queued、failed 或 no-op 结果伪装成已完成高风险操作。
...
governed execution、durable runtime 与 provisioning / recovery 的限制，以对应 example / smoke 条目和 [docs/PraxisHighRiskCapabilitySafety.md](./docs/PraxisHighRiskCapabilitySafety.md)、[docs/PraxisDurableRuntimeGuide.md](./docs/PraxisDurableRuntimeGuide.md) 为准。
当前 `tap.project(...).provision(...)` 返回的是 host-neutral staged receipt：它不会执行真实安装副作用，但会把 bundle / activation / replay 证据写入 TAP checkpoint 与 recovery readback；`tap.project(...).provisioning()` 则用于单独读取 durable provisioning state。
...
- generate / stream reuse the current provider conversation lane
...
- example: `swift run PraxisRuntimeKitGovernedExecutionExample`
- example: `swift run PraxisRuntimeKitSearchExample`
- smoke: `swift run PraxisRuntimeKitSmoke --suite capabilities`
- smoke: `swift run PraxisRuntimeKitSmoke --suite code`
- smoke: `swift run PraxisRuntimeKitSmoke --suite code-sandbox`
- smoke: `swift run PraxisRuntimeKitSmoke --suite code-patch`
- smoke: `swift run PraxisRuntimeKitSmoke --suite shell`
- smoke: `swift run PraxisRuntimeKitSmoke --suite shell-approval`
...
| `capabilities.generate(...)` / `stream(...)` | ready | ready | 当前复用本地 provider conversation lane；`stream` 是 bounded projected stream，不宣称 token transport |
| `capabilities.embed(...)` | ready | ready | 当前复用本地 embedding baseline |
| `capabilities.describeCodeSandbox(...)` | declared-only contract | placeholder contract | 这是 caller-visible declared execution contract。macOS 当前返回 readable/writable roots 与 enforcement mode 供调用方判断边界，但这不是 OS-level isolation guarantee；Linux 仅返回 placeholder contract truth。 |
| `capabilities.patchCode(...)` | ready | unavailable | 这是 risky but controlled workspace patch surface。macOS 当前只通过 bounded workspace writer lane 应用单文件 patch；Linux 今天仍不暴露该 capability。 |
| `capabilities.runCode(...)` | ready | placeholder-backed bounded seam | 这是 risky but controlled bounded code execution。macOS 当前执行 bounded Swift snippet 并保留 runtime / risk labels；Linux 保留同一结果形状，但诚实返回 placeholder `failedToLaunch` truth。 |
| `capabilities.requestShellApproval(...)` / `readbackShellApproval(...)` | ready | ready | 这是 bounded shell execution 的 durable approval/readback contract。当前 approval state 通过 CMP/TAP 持久化，可 fresh-client recovery 后读回，因此 approval evidence 是 durable and recoverable，而不是仅存在内存里。 |
| `capabilities.runShell(...)` | ready | placeholder-backed bounded seam | 这是 risky but controlled bounded shell execution。macOS 当前走真实本地 shell，并与 approval/readback lane 配套；Linux 保留 bounded result shape，但诚实返回 placeholder `failedToLaunch` truth。 |
| `capabilities.listSkills(...)` / `activateSkill(...)` | ready | ready | 这是 provider-backed skill registry / activation surface，不是任意字符串入口。`activateSkill` 只接受已注册 skill key，且 audit/readback truth 只在 successful outcomes 后追加；Linux 共享同一 provider contract。 |
...
| `capabilities.callTool(...)` / `uploadFile(...)` / `submitBatch(...)` | ready | ready | 这是 bounded provider/local adapter surface。`tool.call` 只接受已注册 MCP tool name，`uploadFile` / `submitBatch` 复用 shipped file-store / batch lanes，且 provider audit truth 只在 successful outcomes 后追加；Linux 仅在各自 documented lane 可用范围内保持同一 contract。 |
