...
- session.open exposes the caller-scoped runtime session header
- search.web / fetch / ground reuse the current deterministic local search baseline
- tap.inspect uses current TAP status/history, capability inventory, recent provider activity, and checkpoint/journal readback for reviewer context
- reviewWorkbench combines TAP inspection, TAP overview, CMP overview, and reviewer queue into one project-scoped reviewer surface
